...
async def get_kandinsky5_queue() -> dict[str, Any]:
    """
    Get the current Kandinsky-5 task queue.

    Returns:
        Dictionary containing:
        - tasks: List of tasks in queue order (currently processing task appears first)
        - total_queued: Number of pending tasks waiting in queue
        - current_task_id: Task ID of currently processing task (or None)
...
async def cancel_kandinsky5_task(task_id: str) -> dict[str, Any]:
    """
    ...
    Returns:
        ...
        - message: Human-readable message about the cancellation

    Raises:
        RuntimeError: If API call fails or task cannot be cancelled
    """
    ...
    try:
        ...
        # Check if it's a 400 error (cannot cancel) or 404 (not found)
